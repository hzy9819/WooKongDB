...
#include "tdb/session_processor.h"	/* [williamcliu] */
#include "tdb/rangestatistics.h"	/* [hongyaozhao] */
#include "tdb/range_plan.h"			/* [hongyaozhao] */
#include "tdb/rangecache.h"			/* [hongyaozhao] */
...
CreateSharedMemoryAndSemaphores(int port)
{
	...
	if (!IsUnderPostmaster)
	{
		...
		/* [williamcliu] test shared memory */
		...
		size = add_size(size, RangeListQueueShmemSize());
		/* [charleyzhao] read/write set for occ*/
		size = add_size(size, ReadWriteSetShmemSize());
		size = add_size(size, getltsShmemSize());
		size = add_size(size, getHLCShmemSize());
		size = add_size(size, KeyXidsHashTableShmemSize());
#ifdef EXEC_BACKEND
	...
	/*
	 * [williamcliu] Set up KVEngine Pid Table
	 */
	KVEnginePidTableShmemInit();
	...
	/*
	 * [charleyzhao] allocate rwset in shared memory
	 */
	CreateReadWriteSetArray();
	InitKeyXidsHashTable();
