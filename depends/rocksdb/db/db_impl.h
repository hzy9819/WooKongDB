...
class DBImpl : public DB {
 public:
  ...
  virtual Status Get(const ReadOptions& options,
                     ColumnFamilyHandle* column_family, const Slice& key,
                     PinnableSlice* value, uint64_t *cts = nullptr, uint64_t *nts = nullptr) override;

  // Function that Get and KeyMayExist call with no_io true or false
  // Note: 'value_found' from KeyMayExist propagates here
  Status GetImpl(const ReadOptions& options, ColumnFamilyHandle* column_family,
                 const Slice& key, PinnableSlice* value, uint64_t *cts = nullptr, uint64_t *nts = nullptr,
  ...
  const Snapshot* GetSnapshotForWriteConflictBoundary(const SequenceNumber seq);
 ...
 protected:
  ...
  Status RTSWriteImpl(const WriteOptions& options, WriteBatch* updates,
                   WriteCallback* callback = nullptr,
                   uint64_t* log_used = nullptr, uint64_t log_ref = 0,
                   bool disable_memtable = false, uint64_t* seq_used = nullptr,
                   size_t batch_cnt = 0,
  ...
  Status ConcurrentWriteImpl(const WriteOptions& options, WriteBatch* updates,
                             WriteCallback* callback = nullptr,
                             uint64_t* log_used = nullptr, uint64_t log_ref = 0,
                             bool disable_memtable = false, uint64_t* seq_used = nullptr,
                             size_t batch_cnt = 0,
 ...
 private:
  ...
  SnapshotImpl* GetSnapshotImpl(const SequenceNumber seq, bool is_write_conflict_boundary);
