...
class DBIter final: public Iterator {
 public:
  ...
  virtual uint64_t cts() const override {
    assert(valid_);
    return seq_;
  }
  virtual uint64_t nts() const override {
    assert(valid_);
    return nseq_;
  }
...
inline uint64_t ArenaWrappedDBIter::cts() const { return db_iter_->cts(); }
inline uint64_t ArenaWrappedDBIter::nts() const { return db_iter_->nts(); }
