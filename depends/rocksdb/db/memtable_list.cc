...
bool MemTableListVersion::Get(const LookupKey& key, std::string* value,
  ...
  return GetFromList(&memlist_, key, value, s, merge_context,
                     max_covering_tombstone_seq, seq, &nseq, read_opts, callback,
                     is_blob_index);
}
...
bool MemTableListVersion::Get(const LookupKey& key, std::string* value,
                              Status* s, MergeContext* merge_context,
                              SequenceNumber* max_covering_tombstone_seq,
                              SequenceNumber* seq, SequenceNumber* nseq, 
...
bool MemTableListVersion::GetFromHistory(
  ...
  return GetFromList(&memlist_history_, key, value, s, merge_context,
                     max_covering_tombstone_seq, seq, &nseq, read_opts,
...
bool MemTableListVersion::GetFromHistory(
    const LookupKey& key, std::string* value, Status* s,
    MergeContext* merge_context, SequenceNumber* max_covering_tombstone_seq,
    SequenceNumber* seq, SequenceNumber* nseq, const ReadOptions& read_opts, bool* is_blob_index) {
  return GetFromList(&memlist_history_, key, value, s, merge_context,
                     max_covering_tombstone_seq, seq, nseq, read_opts,
                     nullptr /*read_callback*/, is_blob_index);
}
...
bool MemTableListVersion::GetFromList(
    ...
    Status* s, MergeContext* merge_context,
    SequenceNumber* max_covering_tombstone_seq, SequenceNumber* seq, SequenceNumber* nseq,
  ...
  for (auto& memtable : *list) {
    ...
    bool done =
        memtable->Get(key, value, s, merge_context, max_covering_tombstone_seq,
                      &current_seq, nseq, read_opts, callback, is_blob_index);
