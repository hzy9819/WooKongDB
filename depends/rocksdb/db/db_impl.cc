...
Status DBImpl::Get(const ReadOptions& read_options,
                   ...
                   PinnableSlice* value, uint64_t *cts, uint64_t *nts) {
  return GetImpl(read_options, column_family, key, value, cts, nts);
}

Status DBImpl::GetImpl(const ReadOptions& read_options,
                       ColumnFamilyHandle* column_family, const Slice& key,
                       PinnableSlice* pinnable_val, uint64_t *cts, uint64_t *nts, 
                       bool* value_found,
  ...
  if (!skip_memtable) {
    ...
    SequenceNumber nseq = kMaxSequenceNumber;
    if (sv->mem->Get(lkey, pinnable_val->GetSelf(), &s, &merge_context,
                     &max_covering_tombstone_seq, &seq, &nseq, read_options, callback,
                     is_blob_index)) {
      done = true;
    ...
    if (!done && !s.ok() && !s.IsMergeInProgress()) {
      ...
      if (cts != nullptr && seq != kMaxSequenceNumber)
        *cts = seq;
      if (nts != nullptr && nseq != 0)
        *nts = nseq;
      return s;
    }
    if (cts != nullptr && seq != kMaxSequenceNumber)
...
bool DBImpl::KeyMayExist(const ReadOptions& read_options,
  ...
  PinnableSlice pinnable_val;
  auto s = GetImpl(roptions, column_family, key, &pinnable_val, nullptr /*cts*/, nullptr /*nts*/, value_found);
  value->assign(pinnable_val.data(), pinnable_val.size());
...
const Snapshot* DBImpl::GetSnapshotForWriteConflictBoundary(const SequenceNumber seq){
  return GetSnapshotImpl(seq, true);
}
#endif  // ROCKSDB_LITE

SnapshotImpl* DBImpl::GetSnapshotImpl(const SequenceNumber seq, bool is_write_conflict_boundary) {
  int64_t unix_time = 0;
  ...
  return snapshots_.New(s, seq, unix_time, is_write_conflict_boundary);
}
...
Status DBImpl::GetLatestSequenceForKey(SuperVersion* sv, const Slice& key,
  ...
  *found_record_for_key = false;
  ...
  sv->mem->Get(lkey, nullptr, &s, &merge_context, &max_covering_tombstone_seq,
               seq, &nseq, read_options, nullptr /*read_callback*/, is_blob_index);
  ...
  if (!cache_only) {
    ...
    sv->current->Get(read_options, lkey, nullptr, &s, &merge_context,
                     ...
                     found_record_for_key, seq, &nseq, nullptr /*read_callback*/,
                     is_blob_index);
