...
void rocksdb_iter_ts(rocksdb_iterator_t* iter, uint64_t *cts, uint64_t *nts) {
  *cts = iter->rep->cts();
  *nts = iter->rep->nts();
}
...
void rocksdb_readoptions_set_sequence(
    rocksdb_readoptions_t* opt,
    const uint64_t sequence) {
  opt->rep.SetSequence(sequence);
}
...
void rocksdb_transaction_commit_with_ts(
    rocksdb_transaction_t* txn, char** errptr, uint64_t commit_ts){
  SaveError(errptr, txn->rep->CommitWithTS(commit_ts));
}

void rocksdb_transaction_commit_with_lts(
    rocksdb_transaction_t* txn, char** errptr, uint64_t commit_ts, uint32_t lts){
  SaveError(errptr, txn->rep->CommitWithLTS(commit_ts, lts));
}
...
char* rocksdb_transaction_get_ts(rocksdb_transaction_t* txn,
                              const rocksdb_readoptions_t* options,
                              const char* key, size_t klen, size_t* vlen, 
  ...
  char* result = nullptr;
  std::string tmp;
  Status s = txn->rep->Get(options->rep, Slice(key, klen), &tmp, cts, nts);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
...
char* rocksdb_transaction_get_cf_ts(rocksdb_transaction_t* txn,
                                 const rocksdb_readoptions_t* options,
                                 rocksdb_column_family_handle_t* column_family,
                                 const char* key, size_t klen, size_t* vlen, 
                                 uint64_t *cts, uint64_t *nts, 
  ...
  std::string tmp;
  Status s =
      txn->rep->Get(options->rep, column_family->rep, Slice(key, klen), &tmp, cts, nts);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
...
char* rocksdb_transaction_get_for_update_ts(rocksdb_transaction_t* txn,
                                         const rocksdb_readoptions_t* options,
                                         const char* key, size_t klen,
                                         size_t* vlen, unsigned char exclusive,
                                         uint64_t *cts, uint64_t *nts,
  ...
  std::string tmp;
  Status s =
      txn->rep->GetForUpdate(options->rep, Slice(key, klen), &tmp, exclusive, cts, nts);
  if (s.ok()) {
    *vlen = tmp.size();
    result = CopyString(tmp);
  } else {
...
rocksdb_transaction_t* rocksdb_optimistictransaction_begin_with_ts(
    rocksdb_optimistictransactiondb_t* otxn_db,
    const rocksdb_writeoptions_t* write_options,
    const rocksdb_optimistictransaction_options_t* otxn_options,
    rocksdb_transaction_t* old_txn,
  ...
  if (old_txn == nullptr) {
    rocksdb_transaction_t* result = new rocksdb_transaction_t;
    result->rep = otxn_db->rep->BeginTransactionWithTS(write_options->rep,
                                                 otxn_options->rep, nullptr, start_ts);
    return result;
  }
  old_txn->rep = otxn_db->rep->BeginTransactionWithTS(
      write_options->rep, otxn_options->rep, old_txn->rep, start_ts);
  return old_txn;
