...
class BlobDB : public StackableDB {
 public:
  ...
  virtual Status Get(const ReadOptions& options,
                     ColumnFamilyHandle* column_family, const Slice& key,
                     PinnableSlice* value, uint64_t *cts = nullptr, uint64_t *nts = nullptr) override = 0;

  // Get value and expiration.
  virtual Status Get(const ReadOptions& options,
                     ColumnFamilyHandle* column_family, uint64_t* expiration, 
					 const Slice& key, PinnableSlice* value) = 0;
  virtual Status Get(const ReadOptions& options, uint64_t* expiration, const Slice& key,
                     PinnableSlice* value) {
    return Get(options, DefaultColumnFamily(), expiration, key, value);
  }
