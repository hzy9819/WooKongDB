...
class TransactionBaseImpl : public Transaction {
 public:
  ...
  Status Get(const ReadOptions& options, ColumnFamilyHandle* column_family,
             ...
             uint64_t *cts, uint64_t *nts) override;
  ...
  Status Get(const ReadOptions& options, const Slice& key,
             std::string* value, uint64_t *cts, uint64_t *nts) override {
    return Get(options, db_->DefaultColumnFamily(), key, value, cts, nts);
  }

  using Transaction::GetForUpdate;
  Status GetForUpdate(const ReadOptions& options,
                      ColumnFamilyHandle* column_family, const Slice& key,
                      std::string* value, bool exclusive, uint64_t *cts, uint64_t *nts) override;

  Status GetForUpdate(const ReadOptions& options,
                      ColumnFamilyHandle* column_family, const Slice& key,
                      PinnableSlice* pinnable_val, bool exclusive, uint64_t *cts, uint64_t *nts) override;
  ...
  //virtual void SetSafeSnapshot(const int64_t seq) override;
  virtual void SetSnapshot() override;
  virtual void SetSnapshotWithTS(const SequenceNumber seq) override;
 ...
 protected:
  ...
  void SetCommitTS(const uint64_t commit_ts) { commit_ts_ = commit_ts;}
  uint64_t GetCommitTS() const { return commit_ts_; }
