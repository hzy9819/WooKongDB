...
/*void TransactionBaseImpl::SetSafeSnapshot(const int64_t seq) {
  //snapshot_ = TDTransactionClassArgsFactory<SnapshotImpl,uint64_t>::create(seq);
  const Snapshot* snapshot = dbimpl_->GetSnapshotForWriteConflictBoundary(seq);
...
void TransactionBaseImpl::SetSnapshotWithTS(const SequenceNumber seq) {
  const Snapshot *snapshot = dbimpl_->GetSnapshotForWriteConflictBoundary(seq);
...
Status TransactionBaseImpl::Get(const ReadOptions& read_options,
  ...
  auto s = Get(read_options, column_family, key, static_cast<PinnableSlice*>(&pinnable_val));
...
Status TransactionBaseImpl::Get(const ReadOptions& read_options,
  ...
  PinnableSlice pinnable_val(value);
  assert(!pinnable_val.IsPinned());
  auto s = Get(read_options, column_family, key, static_cast<PinnableSlice*>(&pinnable_val), cts, nts);
  if (s.ok() && pinnable_val.IsPinned()) {
    value->assign(pinnable_val.data(), pinnable_val.size());
  }  // else value is already assigned
  return s;
...
Status TransactionBaseImpl::Get(const ReadOptions& read_options,
                                ...
                                uint64_t *cts, uint64_t *nts) {
  return write_batch_.GetFromBatchAndDB(db_, read_options, column_family, key,
                                        cts, nts, pinnable_val);
}

Status TransactionBaseImpl::GetForUpdate(const ReadOptions& read_options,
                                         ColumnFamilyHandle* column_family,
                                         const Slice& key, std::string* value,
                                         bool exclusive, uint64_t *cts, uint64_t *nts) {
  ...
  if (s.ok() && value != nullptr) {
    ...
    PinnableSlice pinnable_val(value);
    assert(!pinnable_val.IsPinned());
    s = Get(read_options, column_family, key, &pinnable_val, cts, nts);
    if (s.ok() && pinnable_val.IsPinned()) {
      value->assign(pinnable_val.data(), pinnable_val.size());
    }  // else value is already assigned
  }
