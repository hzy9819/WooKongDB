...
Status WritePreparedTxn::Get(const ReadOptions& read_options,
                             ...
                             const Slice& key, PinnableSlice* pinnable_val, uint64_t *cts, uint64_t *nts) {
...
Status WritePreparedTxn::CommitWithoutPrepareInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  // For each duplicate key we account for a new sub-batch
  const size_t batch_cnt = GetWriteBatch()->SubBatchCnt();
  WriteBatch* working_batch = GetWriteBatch()->GetWriteBatch();
  WriteBatchInternal::SetSequence(working_batch, commit_ts);
  working_batch->SetCommitTS(commit_ts);
  if (lts != 0)
    working_batch->SetLTS(lts);
  return CommitBatchInternal(GetWriteBatch()->GetWriteBatch(), batch_cnt);
}
...
Status WritePreparedTxn::CommitInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  ROCKS_LOG_DETAILS(db_impl_->immutable_db_options().info_log,
                    "CommitInternal prepare_seq: %" PRIu64, GetID());
  // We take the commit-time batch and append the Commit marker.
  // The Memtable will ignore the Commit marker in non-recovery mode
...
Status WritePreparedTxn::RollbackInternal() {
  ...
  struct RollbackWriteBatchBuilder : public WriteBatch::Handler {
    ...
    Status Rollback(uint32_t cf, const Slice& key) {
      ...
      s = db_->GetImpl(roptions, cf_handle, key, &pinnable_val, 
                       nullptr /*cts*/, nullptr /*nts*/, &not_used,
                       &callback);
...
void WritePreparedTxn::SetSnapshotWithTS(SequenceNumber commit_ts) {
  // Note: for this optimization setting the last sequence number and obtaining
  // the smallest uncommitted seq should be done atomically. However to avoid
  // the mutex overhead, we call SmallestUnCommittedSeq BEFORE taking the
  // snapshot. Since we always updated the list of unprepared seq (via
  ...
  auto min_uncommitted = wpt_db_->SmallestUnCommittedSeq();
  const bool FOR_WW_CONFLICT_CHECK = true;
  SnapshotImpl* snapshot = dbimpl_->GetSnapshotImpl(commit_ts, FOR_WW_CONFLICT_CHECK);
  assert(snapshot);
  wpt_db_->EnhanceSnapshot(snapshot, min_uncommitted);
  SetSnapshotInternal(snapshot);
}
