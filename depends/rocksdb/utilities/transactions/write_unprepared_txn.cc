...
Status WriteUnpreparedTxn::CommitWithoutPrepareInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  if (unprep_seqs_.empty()) {
    assert(log_number_ == 0);
    assert(GetId() == 0);
    return WritePreparedTxn::CommitWithoutPrepareInternalWithTS(commit_ts, lts);
  }
  ...
  return CommitInternalWithTS(commit_ts, lts);
}
...
Status WriteUnpreparedTxn::CommitInternalWithTS(SequenceNumber commit_ts, uint32_t lts) {
  // TODO(lth): Reduce duplicate code with WritePrepared commit logic.
...
Status WriteUnpreparedTxn::RollbackInternal() {
  ...
  for (const auto& cfkey : write_set_keys_) {
    ...
    for (const auto& key : keys) {
      ...
      bool not_used;
      s = db_impl_->GetImpl(roptions, cf_handle, key, &pinnable_val, 
