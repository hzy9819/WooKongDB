...
#include "util/td_logging.h"
#include "util/td_errno.h"
namespace rocksdb {
...
class BaseDeltaIterator : public Iterator {
 public:
  ...
  uint64_t cts() const override { return base_iterator_->cts(); }
  uint64_t nts() const override { return base_iterator_->nts(); }
...
struct WriteBatchWithIndex::Rep {
  ...
  int SetDupFlag(const uint64_t pos);
...
int WriteBatchWithIndex::Rep::SetDupFlag(const uint64_t pos)
{
  int ret = 1;

  if (pos > write_batch.rep_.size()) {
    /*TRANS_LOG_ERROR("invalid pos:%lu rep size:%lu", pos, write_batch.rep_.size());*/
    ret = 0;
  } else {
    const int8_t value_type = write_batch.rep_[pos];
    write_batch.rep_[pos] = value_type | 0x80;
    /*TRANS_LOG_DEBUG("SetDupFlag pos:%lu value_type:%u new value_type:%u",
        pos, value_type, (uint8_t)write_batch.rep_[pos]);*/
  }
...
bool WriteBatchWithIndex::Rep::UpdateExistingEntryWithCfId(
  ...
  //int ret = 1;
  if (SetDupFlag(non_const_entry->offset) != 1) {
    /*TRANS_LOG_ERROR("SetDupFlag failed ret:%d key:%s old_offset:%lu new offset:%lu",
        ret, key.TO_HEX(), non_const_entry->offset, last_entry_offset);*/
    //TOOD: report error
  }
...
Status WriteBatchWithIndex::GetFromBatchAndDB(DB* db, 
                                              ...
                                              PinnableSlice* pinnable_val)
{
  return GetFromBatchAndDB(db, read_options, column_family, key, pinnable_val, nullptr, nullptr, nullptr);
}
...
Status WriteBatchWithIndex::GetFromBatchAndDB(DB* db,
  ...
  return GetFromBatchAndDB(db, read_options, column_family, key, pinnable_val, cts, nts,
                           nullptr);
}

Status WriteBatchWithIndex::GetFromBatchAndDB(
    DB* db, const ReadOptions& read_options, ColumnFamilyHandle* column_family,
    const Slice& key, PinnableSlice* pinnable_val, uint64_t *cts, uint64_t *nts, ReadCallback* callback) {
  Status s;
  MergeContext merge_context;
  ...
  if (!callback) {
    s = db->Get(read_options, column_family, key, pinnable_val, cts, nts);
  } else {
    s = static_cast_with_check<DBImpl, DB>(db->GetRootDB())
            ->GetImpl(read_options, column_family, key, pinnable_val, cts, nts, nullptr,
                      callback);
  }
